...
def get_auth_header_credentials(header: str, prefix: Literal["Bearer", "Service"]) -> str:
...
def create_jwt(subject: str, expiration_time_minutes: int, secret_key: str) -> str:
    expiration_time = datetime.datetime.now(datetime.timezone.utc) + datetime.timedelta(minutes=expiration_time_minutes)

    data_to_encode = {"exp": expiration_time, "subject": subject}

    jwt = jose.jwt.encode(data_to_encode, secret_key)
    return jwt


def decode_jwt(jwt: str, secret_key: str) -> dict[Literal["subject"], str]:
    """
    ### Raises:
    ...
    """
    try:
        payload = jose.jwt.decode(jwt, secret_key)
...
def service_auth(authorization: str = Header()) -> None:
    if get_auth_header_credentials(authorization, "Service") != config.auth.service_token.get_secret_value():
        raise response.ErrorResponse(
            code=response.ErrorCodes.ACCESS_DENIED, http_status_code=http_status.HTTP_401_UNAUTHORIZED
        )
