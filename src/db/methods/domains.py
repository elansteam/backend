...
def attach_to_entity(
    domain: str, target_id: int, target_type: types.EntityTargetType, session: ClientSession | None = None
) -> None:
    domains.find_one_and_update(
        {"_id": domain}, {"$set": {"target_id": target_id, "target_type": target_type}}, session=session, upsert=True
    )
