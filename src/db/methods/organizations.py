...
def get(organization_id: int, session: ClientSession | None = None) -> types.Organization | None:
    if (organization := organizations.find_one({"_id": organization_id}, session=session)) is None:
        return None
...
def insert_organization(organization: types.OrganizationWithoutID, session: ClientSession | None = None) -> int:
...
def get_organizations_by_user(user_id: int, session: ClientSession | None = None) -> list[types.Organization]:
    return [
        ...
        for org in organizations.aggregate([{"$match": {"members": {"$elemMatch": {"id": user_id}}}}], session=session)
    ]
